...
class Server {
  ...
  setupHooks() {
    ...
    const addHooks = (compiler) => {
      const { compile, invalid, done } = compiler.hooks; // 监听webpack的done钩子，tapable提供的监听方法
  ...
  setupDevMiddleware() {
    ...
    /**
     * webpack-dev-server只负责启动服务和前置准备工作，
     * 所有文件相关的操作都抽离到webpack-dev-middleware库了，
     * 主要是本地文件的编译和输出以及监听，使职责的划分更清晰
     * 
     * webpack-dev-middleware 中，compiler.watch方法功能非常强大，对文件监听变化-->编译、打包-->继续监听
     * 监听本地文件的变化主要是通过 文件的生成时间 是否有变化
     */
  ...
  _sendStats(sockets, stats, force) {
    ...
    this.sockWrite(sockets, 'hash', stats.hash); // 通过websocket给浏览器发送事件通知，hash errors warnings ok 事件；
